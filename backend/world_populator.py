...
# Sehr seltene Welt-Highlights (für alle walkable biome)
RARE_SCATTER = [
...
def _pick_for_tile(world, x: int, y: int, tile_id: int) -> str | None:
    ...
    for prop_type, base, density_kind in biome:
        ...
        # In trockenen/unfruchtbaren Biomen: fertility ist niedriger
        # In fruchtbaren: höher
        adjusted = base * _density_mod(density) * (0.7 + fertility * 0.6)
        if random.random() < adjusted:
            return prop_type
    # Rare scatter über alle biome
...
async def populate_chunk_if_needed(world, structure_manager, connection_manager,
    ...
    for ly in range(CHUNK_SIZE):
        for lx in range(CHUNK_SIZE):
            ...
            wy = cy * CHUNK_SIZE + ly
            tile = chunk[ly][lx]
            # Nicht walkable → nichts platzieren
